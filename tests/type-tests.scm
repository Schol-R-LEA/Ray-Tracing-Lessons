...
(test-with-runner 
  ...
  (test-group
   ...
   (test-group 
    ...
    (test-assert (colors-=? rgb-red rgb-red))
    (test-assert (not (colors-=? rgb-red rgb-green))))
#|
      ...
      (do ((matrix (permute-list test-colors))
        ...
        (do ((j 0 (+ j 1)))
          ...
          (let* ((test-color (list-ref (list-ref matrix i) j))
                ...
                (test-result (colors-=? test-color comparison-color)))
...
|#
   ...
   (test-group 
    ...
    (let ((magenta (blend-colors rgb-red rgb-blue)))
      ...
      (test-assert (colors-=? magenta rgb-magenta)))
    ...
    (let ((yellow (blend-colors rgb-red rgb-green)))
      ...
      (test-assert (colors-=? yellow rgb-yellow))

      (let ((maroon (add-colors rgb-red rgb-blue)))
        (test-equal #xff (red-of maroon))
        (test-equal 0 (green-of maroon))
        (test-equal #xff (blue-of maroon))
        (test-assert (colors-=? maroon rgb-maroon)))
      ...
      (let ((maroon (make-rgb-color #xff 0 #xff))
        ...
        (test-assert (colors-=? maroon red-blue)))))))
