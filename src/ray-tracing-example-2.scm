...
(define (trace-image nx ny color-range)
  (let ((ppm (make-ppm-record (open-output-file
                               (date->string (current-date 0) "simple-sphere-~5.ppm"))
                              (exact (floor nx))
                              (exact (floor ny))
                              (+ color-range 0.99)
                              ""))     
...
(define (color ray color-range r g b)
  (if (hit-sphere (make-point3D 0 0 1) 0.5 ray)
      rgb-red
...
(define (hit-sphere sphere-center radius ray)
  (let* ((center-offset (point3D-subtract (origin-get ray) sphere-center))
         (direction (direction-get ray))
